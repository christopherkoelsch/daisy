...
void followerBig::setInitialCondition(ofImage &Daisy, ofImage &Petal, ofImage &ImageHovered, float px, float py, float vx, float vy){
    ...
    for (int i=0; i<PETALS_NUMBER; i++) {
        ...
        cout<<i%2<<endl;
        myPetals.back().setup(Petal, ImageHovered, petalX,petalY,ang, touch);
    }
...
void followerBig::update(){
    ...
    if(app->handPostions.size()>0){
        for (int i=0; i< app->handPostions.size(); i++) {
            float dis = app->handPostions[i].distance(pos);
            if(bPetalFixed){
                if (dis<136 ) {
                    for (int i=0; i<myPetals.size()-5; i++) {
    ...
    for (int i=0; i<myPetals.size(); i++) {
        ...
        if (!myPetals[i].bFly) {
            ...
            if(myPetals[i].isRepulsionForce){
                testApp * app = (testApp *)ofGetAppPtr();
                if(app->handPostions.size()>0){
                    for (int i=0; i< app->handPostions.size(); i++) {
                        myPetals[i].addRepulsionForce(app->handPostions[i].x,
                                                      app->handPostions[i].y,
                                                      75,
                                                      10.0);
                    }
                }
